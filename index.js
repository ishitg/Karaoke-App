...
const yourBearerToken =
  "QJmAGsBGBN6HU5Pbt6Np7WcEcRxhXMXTAkBDbIeV20tOn5uyUgo0jkW1Jmn8EOqs";
...
app.post("/submit", async (req, res) => {
  ...
  try {
    const searchresponse = await axios.get(
      `https://api.genius.com/search?q=${songTitle}`,
      {
        headers: {
    ...
    if (hits.length > 0) {
      ...
      const songresp = await axios.get(
      ...
      if (
        hitter.media &&
        hitter.media.some((media) => media.provider === "spotify")
      ) {
        const spotifyMedia = hitter.media.find(
            ...
            "grant_type=client_credentials",
            {
              headers: {
                ...
                Authorization:
                  "Basic " +
                  Buffer.from(
                    "035da09cd08b4ece908059f26cfd393c" +
                      ":" +
                      "10613cb874d5479da167729a61bee2d4"
                  ).toString("base64"),
              },
            }
          );
        ...
        async function searchSongOnSpotify(songTitle, accessToken) {
          ...
          const response = await axios.get(
            `https://api.spotify.com/v1/search?q=${songTitle}&type=track`,
            {
              headers: {
